    ...
    <div class="admin-container">
      ...
      <div v-if="activeTab === 'dispatch'" class="tab-content">
        <div class="admin-card">
          ...
          <div v-if="dispatchSelectedExam" style="margin-top: 20px;">
            ...
            <div class="table-responsive"><table class="data-table">
              <thead><tr><th style="width: 40px; text-align: center;"><input type="checkbox" class="custom-checkbox" :checked="isAllDispatchSelected" @change="toggleAllDispatch" /></th><th>實習單位</th><th>學員姓名</th><th>派發狀態</th></tr></thead>
              <tbody>
                <tr v-for="student in dispatchFilteredStudents" :key="student.id" :class="{'selected-row': dispatchSelectedStudents.includes(student.id)}">
                  ...
                  <td><span v-if="hasDispatched(dispatchSelectedExam, student.id)" style="color: #2ecc71; font-weight: bold;">✅ 已派發</span><span v-else style="color: #95a5a6;">尚未派發</span></td>
                </tr>
        ...
        <div class="admin-card" style="margin-top: 20px;">
          <h3>🔓 已派發測驗之解答管理</h3>
          <p class="desc">在確認所有學員皆完成測驗後，在此處「一鍵公開」正確解答。</p>
          <div class="table-responsive"><table class="data-table">
            ...
            <tbody>
              <tr v-for="stat in dispatchStats" :key="stat.exam_id">
                ...
                <td style="text-align: center;"><button v-if="!stat.show_answers" @click="toggleAnswersVisibility(stat.exam_id, stat.show_answers)" class="btn success-btn small-btn">一鍵公開解答</button><button v-else @click="toggleAnswersVisibility(stat.exam_id, stat.show_answers)" class="btn danger-btn small-btn">關閉解答</button></td>
              </tr>
      ...
      <div v-if="activeTab === 'exams'" class="tab-content">
        ...
        <div class="admin-card preview-card" v-else>
          <div class="card-header-flex align-center">
            ...
            <div class="action-row" style="margin-top: 0;"><button @click="cancelPreview" class="btn danger-btn small-btn">{{ editingExamId ? '取消編輯' : '取消匯入' }}</button><button @click="confirmSaveExam" class="btn primary-btn small-btn">✅ 確認無誤並儲存</button></div>
          </div>
          <div class="form-row" style="background: #fdfdfd; padding: 15px; border-radius: 8px; border: 1px solid #eee;">
            <div class="form-group"><label>測驗卷名稱：</label><input type="text" v-model="previewExamTitle" class="form-input" /></div>
            <div class="form-group"><label>測驗類型：</label><select v-model="previewExamType" class="form-input"><option value="pre_test">課前測驗</option><option value="post_test">課後測驗</option></select></div>
          </div>
          ...
          <div class="preview-questions-list">
            <div v-for="(q, qIndex) in previewQuestions" :key="qIndex" class="question-item">
              <div class="q-header"><span class="q-num">Q{{ qIndex + 1 }}</span><input v-model="q.text" class="form-input q-text-input" /><button @click="removePreviewQuestion(qIndex)" class="btn danger-btn small-btn" title="刪除此題">🗑️</button></div>
              <ul class="q-options">
        ...
        <div class="admin-card">
          <h3>📋 目前已建立的測驗卷清單</h3>
          <div class="table-responsive"><table class="data-table">
            <thead><tr><th>測驗卷名稱</th><th>測驗類型</th><th>建立時間</th><th style="width: 180px; text-align: center;">操作</th></tr></thead>
            <tbody>
      ...
      <div v-if="activeTab === 'users'" class="tab-content">
        <div class="admin-card">
          <div class="card-header-flex">
            <div class="card-header-text"><h3>📊 Excel 批次匯入帳號</h3><p class="desc">請依照系統報表格式上傳。新增「實習單位」欄位以便分流管理測驗。</p></div>
        ...
        <div class="admin-card">
          ...
          <form @submit.prevent="createUser" class="create-form">
            <div class="form-row">
              ...
              <div class="form-group"><label>身分：</label><select v-model="newUser.role" class="form-input"><option value="student">受訓人員 (學員)</option><option value="teacher">臨床指導老師</option><option value="supervisor">單位主管</option><option value="admin">系統管理員</option></select></div>
        ...
        <div class="admin-card">
          ...
          <div class="table-responsive"><table class="data-table">
            <thead><tr><th style="width: 40px; text-align: center;"><input type="checkbox" class="custom-checkbox" :checked="isAllSelectedOnPage" @change="toggleSelectAllOnPage" /></th><th>單位</th><th>姓名</th><th>Email</th><th>身分角色</th><th style="width: 140px; text-align: center;">操作</th></tr></thead>
            <tbody>
              <tr v-for="user in paginatedUsers" :key="user.id" :class="{'selected-row': selectedUserIds.includes(user.id)}">
                ...
                <td style="text-align: center; white-space: nowrap;"><button @click="editUser(user)" class="btn primary-btn small-btn" style="margin-right: 5px;">編輯</button><button @click="deleteUser(user.id)" class="btn danger-btn small-btn">刪除</button></td>
              </tr>
              <tr v-if="filteredUsers.length === 0"><td colspan="6" class="empty-state">此分類下尚無人員資料</td></tr>
            </tbody>
          </table></div>
          <div class="pagination-controls" v-if="totalPages > 1"><button @click="prevPage" :disabled="currentPage === 1" class="page-btn">上一頁</button><span class="page-info">第 {{ currentPage }} 頁 / 共 {{ totalPages }} 頁</span><button @click="nextPage" :disabled="currentPage === totalPages" class="page-btn">下一頁</button></div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'pairing'" class="tab-content">
        <div class="admin-card">
          ...
          <div class="table-responsive"><table class="data-table pairing-table">
            ...
            <tbody>
              <tr v-for="student in students" :key="student.id">
                ...
                <td><select v-model="assignmentData[student.id].teacher_id" class="pairing-select"><option value="">-- 請選擇指導老師 --</option><option v-for="teacher in teachers" :key="teacher.id" :value="teacher.id">{{ teacher.name }}</option></select></td>
                <td><select v-model="assignmentData[student.id].supervisor_id" class="pairing-select"><option value="">-- 請選擇單位主管 --</option><option v-for="supervisor in supervisors" :key="supervisor.id" :value="supervisor.id">{{ supervisor.name }}</option></select></td>
      ...
      <div v-if="activeTab === 'categories'" class="tab-content">
        <div class="admin-card">
          <h3>➕ 新增訓練類別選項</h3>
          <div class="add-category-row" style="display: flex; gap: 10px; margin-top: 15px;"><input type="text" v-model="newCategoryName" placeholder="請輸入類別名稱" class="form-input" style="flex: 1;" @keyup.enter="addCategory" /><button @click="addCategory" class="btn primary-btn" style="white-space: nowrap;">新增選項</button></div>
        </div>
        <div class="admin-card">
          <h3>📋 目前啟用的訓練類別清單</h3>
          <table class="data-table"><tbody><tr v-for="cat in dynamicCategories" :key="cat.id"><td><strong>{{ cat.name }}</strong></td><td style="text-align: right;"><button @click="deleteCategory(cat.id, cat.name)" class="btn danger-btn small-btn">刪除</button></td></tr></tbody></table>
        </div>
      </div>
            ...
            <div v-for="(q, index) in viewingQuestions" :key="q.id" class="question-item">
              ...
              <div class="q-options">
                <label v-for="(opt, optIndex) in q.options" :key="optIndex" class="opt-label" :class="{'is-correct-preview': opt === q.correct_answer}">
                  <input type="radio" disabled class="custom-radio"><span class="opt-text">{{ opt }}</span><span v-if="opt === q.correct_answer" class="correct-badge" style="margin-left: auto;">正確解答</span>
                </label>
              </div>
            </div>
          </div>
...
async function submitDispatch() { if (dispatchSelectedStudents.value.length === 0) return Swal.fire('提示', '請至少勾選一名學員', 'warning'); Swal.fire({ title: '派發中...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } }); const payload = dispatchSelectedStudents.value.map(sId => ({ exam_id: dispatchSelectedExam.value, student_id: sId, show_answers: dispatchShowAnswers.value })); const { error } = await supabase.from('exam_dispatch').insert(payload); if (error) { Swal.fire('錯誤', error.message, 'error') } else { Swal.fire('成功', '測驗任務派發完畢！', 'success'); dispatchSelectedStudents.value = []; await loadDispatches() } }

const dispatchStats = computed(() => { const stats = {}; dispatchRecords.value.forEach(r => { if (!stats[r.exam_id]) { const exam = examList.value.find(e => e.id === r.exam_id); stats[r.exam_id] = { exam_id: r.exam_id, title: exam ? exam.title : '未知測驗', total: 0, completed: 0, show_answers: r.show_answers, pending_names: [] } } stats[r.exam_id].total++; if (r.is_completed) { stats[r.exam_id].completed++ } else { const student = users.value.find(u => u.id === r.student_id); if (student) stats[r.exam_id].pending_names.push(student.name) } if (r.show_answers) stats[r.exam_id].show_answers = true }); return Object.values(stats) })
function showPendingStudents(stat) { if (stat.pending_names.length === 0) { Swal.fire('提示', '所有學員皆已完成測驗！', 'success'); return } const namesHtml = stat.pending_names.map(name => `<span style="display:inline-block; margin: 5px; padding: 5px 10px; background:#ecf0f1; border-radius:4px; font-weight:bold; color: #2c3e50;">${name}</span>`).join(''); Swal.fire({ title: `尚未交卷名單 (${stat.pending_names.length} 人)`, html: `<div style="text-align: left; margin-top: 15px;">${namesHtml}</div>`, icon: 'info', confirmButtonText: '關閉' }) }
...
function addNewQuestion() { previewQuestions.value.push({ text: '請輸入新題目內容...', options: ['選項A', '選項B', '選項C', '選項D'], correct: '選項A' }) }
async function editExam(exam) { Swal.fire({ title: '載入題目中...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } }); const { data, error } = await supabase.from('questions').select('*').eq('exam_id', exam.id); if (error) return Swal.fire('錯誤', '題目載入失敗', 'error'); editingExamId.value = exam.id; previewExamTitle.value = exam.title; previewExamType.value = exam.type; previewQuestions.value = data.map(q => ({ id: q.id, text: q.question_text, options: q.options, correct: q.correct_answer })); Swal.close(); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  ...
  const reader = new FileReader(); reader.onload = async (e) => {
    try {
      ...
      while ((matchQ = qRegex.exec(questionSection)) !== null) {
        const qNum = parseInt(matchQ[1]); let rawText = matchQ[2].trim(); let options = [], questionText = '', correctText = ''; const ansKey = answersMap[qNum] || 'A'; const idxA = rawText.search(/\s*(?:\(A\)|A\.)\s*/)
...
async function confirmSaveExam() {
  if (!previewExamTitle.value.trim()) return Swal.fire('提示', '名稱不能為空', 'warning'); if (previewQuestions.value.length === 0) return Swal.fire('提示', '考卷內不能沒有題目！', 'warning')
  Swal.fire({ title: '儲存中...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } }); try {
    let finalQuestions = JSON.parse(JSON.stringify(previewQuestions.value)); if (shuffleOptionsMode.value) finalQuestions.forEach(q => { if (q.options.length > 2) q.options = shuffleArray(q.options) }); if (shuffleQuestionsMode.value) finalQuestions = shuffleArray(finalQuestions); let targetExamId = editingExamId.value
    if (targetExamId) {
      ...
      const questionsToUpdate = finalQuestions.filter(q => q.id).map(q => ({ id: q.id, exam_id: targetExamId, question_text: q.text, options: q.options, correct_answer: q.correct })); const questionsToInsert = finalQuestions.filter(q => !q.id).map(q => ({ exam_id: targetExamId, question_text: q.text, options: q.options, correct_answer: q.correct }))
      ...
      targetExamId = examData[0].id; const qPayload = finalQuestions.map(q => ({ exam_id: targetExamId, question_text: q.text, options: q.options, correct_answer: q.correct })); const { error: qErr } = await supabase.from('questions').insert(qPayload); if (qErr) throw qErr
    }
...
async function deleteExam(id, title) { const { isConfirmed } = await Swal.fire({ title: `確定要刪除「${title}」嗎？`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#e74c3c' }); if (!isConfirmed) return; const { error } = await supabase.from('exams').delete().eq('id', id); if (error) return Swal.fire('錯誤', error.message, 'error'); Toast.fire({ icon: 'success', title: '測驗卷已刪除' }); await loadExams() }
async function viewExam(exam) { Swal.fire({ title: '載入中...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } }); const { data, error } = await supabase.from('questions').select('*').eq('exam_id', exam.id); if (error) { Swal.fire('錯誤', '題目載入失敗', 'error'); return } viewingExam.value = exam; viewingQuestions.value = data; isViewingModalOpen.value = true; Swal.close() }
...
async function addCategory() { if (!newCategoryName.value.trim()) return Swal.fire('提示', '請輸入類別名稱', 'warning'); const { error } = await supabase.from('training_categories').insert([{ name: newCategoryName.value.trim() }]); if (error) return Swal.fire('錯誤', error.message, 'error'); newCategoryName.value = ''; Toast.fire({ icon: 'success', title: '類別新增成功' }); await loadCategories() }
async function deleteCategory(id, name) { const { isConfirmed } = await Swal.fire({ title: `確定要刪除「${name}」嗎？`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#e74c3c' }); if (isConfirmed) { await supabase.from('training_categories').delete().eq('id', id); await loadCategories() } }

const currentPage = ref(1); const itemsPerPage = 10; watch(roleFilter, () => { currentPage.value = 1; selectedUserIds.value = [] })
const filteredUsers = computed(() => { return roleFilter.value === 'all' ? users.value : users.value.filter(u => u.role === roleFilter.value) }); const totalPages = computed(() => { return Math.ceil(filteredUsers.value.length / itemsPerPage) || 1 })
...
async function batchDeleteUsers() {
  ...
  Swal.fire({ title: '清理中...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } }); const results = { success: 0, fail: 0 }; const concurrency = 5
  for (let i = 0; i < selectedUserIds.value.length; i += concurrency) { const batch = selectedUserIds.value.slice(i, i + concurrency); await Promise.all(batch.map(async (userId) => { const { error } = await supabase.rpc('delete_user_admin', { target_user_id: userId }); error ? results.fail++ : results.success++ })) }
  if (results.fail === 0) { Swal.fire({ icon: 'success', title: '刪除成功', text: `已成功移除 ${results.success} 名人員`, timer: 1500, showConfirmButton: false }) } else { Swal.fire({ icon: 'warning', title: '部分失敗', text: `成功: ${results.success}，失敗: ${results.fail}` }) }
...
async function deleteUser(userId) { const { isConfirmed } = await Swal.fire({ title: '確定要刪除？', icon: 'warning', showCancelButton: true, confirmButtonColor: '#e74c3c' }); if (!isConfirmed) return; await supabase.rpc('delete_user_admin', { target_user_id: userId }); selectedUserIds.value = selectedUserIds.value.filter(id => id !== userId); await loadUsers() }
...
async function handleFileUpload(event) {
  const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = async (e) => {
    try { const data = new Uint8Array(e.target.result); const workbook = XLSX.read(data, { type: 'array' }); const worksheet = workbook.Sheets[workbook.SheetNames[0]]; const jsonData = XLSX.utils.sheet_to_json(worksheet); Swal.fire({ title: '批次匯入中...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } }); const results = { success: 0, fail: 0, failRows: [] }; for (let i = 0; i < jsonData.length; i += 5) { const batch = jsonData.slice(i, i + 5); await Promise.all(batch.map(async (row) => { if (!row.Email || !row['姓名'] || !row['身分證字號']) { results.fail++; return; } const roleText = row['身分']?.toString().trim() || '學員'; const role = ROLE_MAP[roleText]; const unit = row['實習單位']?.toString().trim() || ''; if (!role) { results.fail++; return; } const { error } = await supabase.functions.invoke('create-user', { body: { email: row.Email, password: row['身分證字號'].toString(), name: row['姓名'], role, unit } }); error ? results.fail++ : results.success++ })) } await loadUsers(); Swal.fire({ icon: 'info', title: '匯入完成', html: `成功: ${results.success} 筆，失敗: ${results.fail} 筆` }) } catch (err) { Swal.fire('錯誤', '檔案解析失敗', 'error') } event.target.value = ''
  }; reader.readAsArrayBuffer(file)
}
async function saveAssignment(studentId) { const data = assignmentData.value[studentId]; if (!data.teacher_id || !data.supervisor_id) { return Swal.fire({ icon: 'warning', title: '提示', text: '請選擇指導老師與單位主管' }) } Swal.fire({ title: '儲存配對中...', allowOutsideClick: false, didOpen: () => { Swal.showLoading() } }); const { error } = await supabase.from('assignments').upsert({ student_id: studentId, teacher_id: data.teacher_id, supervisor_id: data.supervisor_id }, { onConflict: 'student_id' }); if (error) Swal.fire('配對失敗', `資料庫錯誤: ${error.message}`, 'error'); else Swal.fire('成功', '配對已成功儲存！', 'success') }
...
.empty-state { text-align: center; padding: 30px !important; }

.role-badge { padding: 5px 12px; border-radius: 12px; font-size: 13px; font-weight: bold; display: inline-block; }
.role-badge.student { background: #3498db !important; } .role-badge.teacher { background: #9b59b6 !important; }
.role-badge.supervisor { background: #e67e22 !important; } .role-badge.admin { background: #34495e !important; }
...
.primary-btn { background: #3498db; } .secondary-btn { background: #95a5a6; } .success-btn { background: #2ecc71; display: inline-flex; align-items: center; justify-content: center; } .danger-btn { background: #e74c3c; } .dark-btn { background: #2c3e50; }
.btn:hover:not(:disabled) { filter: brightness(0.9); transform: translateY(-1px); } .btn:disabled { background: #bdc3c7; cursor: not-allowed; }
...
.page-btn { padding: 6px 12px; border: 1px solid #bdc3c7; background: white; border-radius: 4px; cursor: pointer; font-weight: bold; transition: 0.2s; }
...
.q-num { font-weight: 900; font-size: 18px; width: 35px; }
