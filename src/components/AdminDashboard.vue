<template>
  <div class="app-wrapper" :class="printMode ? `print-mode-${printMode}` : ''">
    ...
    <div v-if="isScoreModalOpen" class="modal-overlay no-print" @click.self="closeScoreModal">
      <div class="modal-content review-modal" style="max-width: 950px; height: 90vh;">
        ...
        <div class="modal-body" style="height: calc(100% - 70px); overflow-y: auto;">
    ...
    <!-- ======================================================= -->
    <!-- 🖨️ 隱藏版：正式 PDF 匯出格式 (消除預設頁首頁尾、橫向排版、完整不裁切) -->
    <!-- ======================================================= -->
    <div class="print-only-scores" v-show="printMode === 'scores'">
      ...
      <div v-if="scoreReportData.isScale">
        <div v-for="record in filteredScoreRecords" :key="record.studentId" class="scale-page-wrapper">
          <h1 style="text-align: center; font-size: 24px; font-weight: bold; margin: 0 0 10px 0; color: #000; letter-spacing: 2px;">
            護生實習壓力評估與分析報告
          </h1>
                ...
                <table style="width: 100%; border-collapse: collapse; border: 1px solid #000; font-size: 13px; table-layout: fixed; word-wrap: break-word;">
                  <thead>
                    <tr style="background: #f0f0f0;">
                      ...
                      <th style="border: 1px solid #000; padding: 6px; text-align: center; width: 18%;">平均得分</th>
      ...
      <div v-else>
        ...
        <table class="print-meta-table">
          <tbody>
            <tr>
              <td class="print-meta-label">課程/測驗：</td><td class="print-meta-value">{{ scoreReportData.title }}</td>
              <td class="print-meta-label">學年學期：</td><td class="print-meta-value">{{ reportMeta.semester || '______' }}</td>
            </tr>
            <tr>
              <td class="print-meta-label">授課/指導：</td><td class="print-meta-value">{{ reportMeta.teacherName }}</td>
        ...
        <table class="print-score-table">
          <thead>
            <tr>
              ...
              <th style="width: 20%;">學員帳號</th>
              <th style="width: 20%;">姓名</th>
              <th style="width: 30%;">實習單位</th>
    ...
    <div class="printable-demo-printonly" v-show="printMode === 'demo'">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px; border: 1px solid #000; padding: 15px; border-radius: 6px;">
        <p><strong>撰寫學員：</strong> 護理部 - 測試學員</p>
        <p><strong>指導老師：</strong> 臨床指導教師</p>
        <p><strong>訓練類別：</strong> 基層護理人員臨床專業能力訓練</p>
        <p><strong>訓練日期：</strong> 2026-09-14</p>
...
function generateRadarSVG(values6, color = '#1f6f78') {
  ...
  for (let ring = 1; ring <= 5; ring++) {
    ...
    rings += `<polygon points="${pts}" fill="none" stroke="#ccc" stroke-width="1" stroke-dasharray="3,3"/>`;
  }
  
  let axes = '', labels = '';
  const catNames = ["實際護理", "教師護理", "作業工作", "同學生活", "專業知識", "實習環境"];
  catNames.forEach((name, i) => {
    const [x, y] = pt(i, R);
    axes += `<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="#aaa" stroke-width="1"/>`;
    // 將字體往外推 35px，並設定字體大小為 18px (與圖標標題接近)
...
function printSelectedScores() {
  ...
  const restoreState = () => { document.title = originalTitle; printMode.value = ''; window.removeEventListener('afterprint', restoreState); };
...
const dispatchStats = computed(() => { 
  ...
  dispatchRecords.value.forEach(r => { 
    if (!stats[r.exam_id]) { 
      ...
      stats[r.exam_id] = { exam_id: r.exam_id, title: exam ? exam.title : '未知任務', total: 0, completed: 0, show_answers: r.show_answers, type: exam ? exam.type : '', pending_names: [] } 
    } 
    stats[r.exam_id].total++; 
    if (r.is_completed) { stats[r.exam_id].completed++ } 
    else { const student = users.value.find(u => u.id === r.student_id); if (student) stats[r.exam_id].pending_names.push(student.name) } 
...
function calculateScaleDimensions(answersJson, qOrderMap) {
  const result = {};
  for (let dKey in DIMENSIONS_MAP) {
    ...
    dim.qList.forEach(qNum => {
      const qIndex = qNum - 1; 
      const targetUuid = Object.keys(qOrderMap).find(id => qOrderMap[id] === qIndex);
      
      let ansVal = undefined;
      if (answersJson && targetUuid) {
        if (answersJson.grading) { const g = answersJson.grading.find(x => x.id === targetUuid); if (g) ansVal = g.student_answer; } 
        else { ansVal = answersJson[targetUuid]; }
      }
      ...
      if (ansVal !== undefined) {
        ...
        if (typeof ansVal === 'number') scoreNum = ansVal;
          ...
          const sMatch = ansVal.match(/(\d+)分/);
          if (sMatch) scoreNum = parseInt(sMatch[1], 10);
          else {
...
async function openScoreModal(stat) {
  ...
  const { data: qData } = await supabase.from('questions').select('id').eq('exam_id', stat.exam_id).order('id', { ascending: true });
  const qOrderMap = {}; 
  if (qData) { qData.forEach((q, idx) => { qOrderMap[q.id] = idx; }); }
  ...
  const reader = new FileReader(); reader.onload = async (e) => {
    try {
      ...
      while ((matchQ = qRegex.exec(questionSection)) !== null) {
        ...
        if(inlineAnsMatch) { inlineAns = inlineAnsMatch[1].toUpperCase(); rawText = rawText.replace(/^\s*\(?[A-D○×]\)?\s+/, '').trim(); }
...
@media print {
  ...
  .print-meta-table td { border: 1px solid #000 !important; padding: 5px 10px !important; font-size: 13pt !important; }
  ...
  .print-score-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; box-sizing: border-box; }
  ...
  .print-score-table th, .print-score-table td { border: 1px solid #000 !important; padding: 8px !important; font-size: 13pt !important; text-align: left; }
  
  .print-footer { display: flex; justify-content: space-between; font-size: 14pt !important; margin-top: 15px; font-weight: bold; page-break-inside: avoid; }
}
</style>